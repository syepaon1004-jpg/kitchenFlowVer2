...
- 영향받는 다른 파일 목록
- 예상 위험 요소
- CSS 3D 관련 작업이면: transform 순서, preserve-3d 체인, hit-test 방식 명시

출력: "작업 계획: ..."
사용자(지휘관) 확인 후 진행. 확인 전에 구현하지 않는다.
```
...
- MISTAKES.md 업데이트 여부 확인 (에러가 있었다면)
- LEARNINGS.md 업데이트 여부 확인 (새 발견/패턴이 있었다면)
- 위 기록을 안 했으면 지금 해라
...
- Zustand 셀렉터에 인라인 `filter`/`map` 금지
- 기존 코드 스타일과 다른 패턴 도입 금지
- 사용자가 요청하지 않은 리팩토링 금지
- 확인 없이 패키지 설치/삭제 금지

### 아키텍처
- 물리엔진에서 DB write 금지 (Zustand에서만 계산)
- 어드민/게임 컴포넌트 간 import 금지 (shared만 공용)
- CSS 변수 인라인 하드코딩 금지 (gameVariables.css에 정의)
- 재료 인스턴스를 드래그 시작 시점에 생성 금지 (드롭 성공 후에만)

### CSS 3D 전용
- Three.js 등 3D 라이브러리 사용 금지
- `pointer-events` CSS로 3D 공간 클릭 감지 시도 금지 (getBoundingClientRect 사용)
- transform 순서 변경 금지: `translateZ` → `rotateX` (이동 먼저, 방향 나중)
